...
export function TaxRateHistoryDialog({
  taxRateId,
  taxRateName,
}: TaxRateHistoryDialogProps) {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/tax-rates/${taxRateId}/history`);
      if (!response.ok) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>History for {taxRateName}</DialogTitle>
        </DialogHeader>
              ...
              {loading ? (
                ...
                history.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      {item.changedByUser?.name || item.changedBy}
                    </TableCell>
